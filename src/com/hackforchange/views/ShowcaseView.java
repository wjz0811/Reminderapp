...
/**
 * This class allows you to have multiple showcases on one screen, you can then
 * page through each one as you press the 'ok' button
 */
public class ShowcaseView {
	...
	/**
	 * @param activity
	 *            The activity containing the views you wish to showcase
	 * @param showcaseTemplateLayout
	 *            Must be the layout of a ShowcaseView - use this to style your
	 *            showcase
	 */
	public ShowcaseView(Activity activity,
			int showcaseTemplateLayout) {
	...
	public void addView(ItemViewProperties properties) {
		...
		viewTemplate.setShowcaseItem(properties.itemType, properties.id,
				activity);
	...
	private ShowcaseView newInstanceOfShowcaseView() {
		return (ShowcaseView) activity.getLayoutInflater().inflate(
				showcaseTemplateId, null);
	}

	private void setChainClickListener(
			final ShowcaseView viewTemplate) {
	...
	/**
	 * Showcases will be shown in the order they where added, continuing when
